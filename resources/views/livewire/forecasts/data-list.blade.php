    ...
    <table class="min-w-full w-auto">
        <thead>
            <tr class="bg-gray-600 text-white uppercase font-light text-sm">
                ...
                <th class="py-2 px-3 text-left">Current Month</th>
                <th class="py-2 px-3 text-left">Forecast</th>
                <th class="py-2 px-3 text-left">Cut Off Date</th>
                <th class="py-2 px-3 text-left">Remaining Day(s)</th>
                <th class="py-2 px-3 text-left">Period Month(s)</th>
        ...
        <tbody class="text-black text-xs font-light">
            @foreach ($datas as $user)
                <tr class="border border-gray-200 hover:bg-gray-100">
                    ...
                    <td class="py-2 px-3 text-left whitespace-nowrap">{{ $user->cut_off_date ? $user->cut_off_date->format('M-Y') : '-' }}</td>
                    <td class="py-2 px-3 text-left whitespace-nowrap">{{ $user->cut_off_date ? $user->cut_off_date->addMonth()->format('M-Y') : '-' }}</td>
                    <td class="py-2 px-3 text-left whitespace-nowrap">{{ $user->cut_off_date ? $user->cut_off_date->format('d-m-Y') : '-' }}</td>
                    <td class="py-2 px-3 text-left">{{ $user->remaining_days }} day(s) left</td>
                    <td class="py-2 px-3 text-left">Last {{ $user->period_months }} Month(s)</td>
