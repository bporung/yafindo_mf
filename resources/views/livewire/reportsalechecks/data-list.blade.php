    ...
    <table class="min-w-full text-xs w-auto">
        <thead>
            <tr class="bg-gray-600 text-white uppercase font-light text-xs">
                <th class="py-2 px-1 bg-gray-600  text-left sticky left-0 z-10">Customer</th>
                @foreach ($periods as $prKey => $pr)
                <th class="py-2 px-3 text-center"><span class="whitespace-nowrap">{{ $prKey }}</span><br>{{ $pr['day_name'] }}</th>
                @endforeach
        ...
        <tbody class="text-black text-xs font-light">
            ...
            @foreach($resCustomers as $r)
                <tr class="border border-gray-200 hover:bg-gray-100">
                    <td class="py-2 px-3 bg-gray-200  text-left whitespace-nowrap sticky left-0 z-10">{{$r->code}}<br>{{$r->nickname}}</td>
                    @foreach ($periods as $prKey => $pr)
                        @php 
                        $y = 0;
